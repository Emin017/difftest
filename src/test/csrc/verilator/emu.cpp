    ...
    switch (o) {
      case 0:
        switch (long_index) {
          ...
          case 1: args.force_dump_result = true; continue;
...
inline char* Emulator::timestamp_filename(time_t t, char *buf) {
  ...
  char *nanshan_home = getenv("NANSHAN_HOME");
  assert(nanshan_home != NULL);
  int len = snprintf(buf, 1024, "%s/build/%s", nanshan_home, buf_time);
  return buf + len;
}
...
inline char* Emulator::cycle_wavefile(uint64_t cycles, time_t t) {
  ...
  char *nanshan_home = getenv("NANSHAN_HOME");
  assert(nanshan_home != NULL);
  int len = snprintf(buf, 1024, "%s/build/%s_%ld", nanshan_home, buf_time, cycles);
  strcpy(buf + len, ".vcd");
  printf("dump wave to %s...\n", buf);
  return buf;
}
