...
int Difftest::step()
{
    ...
    if (dut->csr.mip != proxy->csr.mip)
    { // Ignore difftest for MIP
    ...
    if (NUM_CORES > 1)
    {
        ...
        if (dut->load[i].fuType == 0xC || dut->load[i].fuType == 0xF)
        {
            ...
            if (realWen && reg_cmp_fail)
            {
                ...
                // dut->load[i].paddr, dut->commit[i].wen, dut->commit[i].wdest, get_commit_data(i), dut->commit[i].pc);
                uint64_t golden;
                int len = 0;
                ...
                { // dut->load[i].fuType == 0xF
                    if (dut->load[i].opType % 2 == 0)
                    {
                        len = 4;
                    }
                    else
                    { // dut->load[i].opType % 2 == 1
                        len = 8;
                    }
                }
